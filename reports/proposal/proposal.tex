...
upstanding member of the community). The relationship between the values and
concepts, while intuitively present, are not immediately obvious.  Specifically,
one can imagine a range of network characteristics, interactions, and temporal
factors that affect the accumulation of karma points. In our work, we will
investigate whether such factors can be identified and consistently analyzed,
and as a result, whether karma values can be predicted.

This topic is motivated by the discussions in class about how users evaluate
each other in social graphs. While studying the pattern of karma accumulation is
one aspect of our study, we would like to see what other explict and implicit
factors could account for a user's karma. In that sense, karma could be regarded
...
\subsection{Everyone's an influencer: quantifying influence on Twitter
  \citep{bakshy2011everyone}}
...
\citet{cha2010measuring} study a more practical implication of
influence -- the ability of a user to spread information to a large network of
nodes. In other words, instead of simply using static reputation scores as a
proxy for influence, they study a user's actual outreach as the real indicator
of influence.

The paper compares three measures of influence of Twitter users: in-degree,
number of retweets earned, and number of mentions. They use the \textit{rank
correlation coefficient} to compare measures of influence:
